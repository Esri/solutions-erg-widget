...
function (declare,
  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin, Evented], {
      ...
      if (!this.test) {
        ...
        this.own(on(this.btnOk, 'click', lang.hitch(this, function () {
          if (!domClass.contains(this.btnOk, 'jimu-state-disabled')) {
            ...
            var ok = false;
            for (var key in this.fields.stats) {
              if (this.fields.stats.hasOwnProperty(key)) {
                ok = true;
              }
            }
            if (!ok) {
    ...
    _completeMapLayers: function (args, test) {
      if (args) {
        ...
        if (typeof (test) === 'undefined') {
          ...
          if (this.fields.url) {
            if (typeof (this.callerTab.fields) !== 'undefined' && this.callerTab.fields) {
              if (this.callerTab.fields.stats) {
                this._setFields(fields);
              } else {
                this._setFields(fields, true);
              }
              var statGroup = this.callerTab.fields.stats;
    ...
    _addTabRow: function (field) {
      ...
      if (this.callerTab.type !== "summary" && numRows >= 10) {
        new Message({
    ...
    updateSummaryType: function () {
      ...
      if (this.callerTab.type !== "summary" && this.callerTab.type !== "groupedSummary") {
        ...
        if (flds.length > 0) {
          this.fields.stats.outFields = flds;
        }
        this.fields.stats.tabCount = this.chk_count_only.checked;
      } else {
        // count
        if (this.callerTab.type === "groupedSummary") {
          this.fields.stats.tabCount = this.chk_count_only.checked;
        } else {
          this.fields.stats.tabCount = this.chk_count.checked;
        }
        if (this.chk_count.checked) {
          //this.summaryLayers[0].stats.count = [
          this.fields.stats.count = [
            {
              value: 0,
        ...
        array.forEach(trs, lang.hitch(this, function (tr) {
          if (typeof (this.fields.stats[tr.selectTypes.value]) === 'undefined') {
            this.fields.stats[tr.selectTypes.value] = [];
          }
          var statBlock = {};
          statBlock.value = 0;
          ...
          this.fields.stats[tr.selectTypes.value].push(statBlock);
        }));
      }
